...
const a_star_algo = async (beg_p,snake_game=false) => {
    ...
    console.log('Start of an a star algorithm!')
    ...
    console.log(row_end,col_end)
        // row = index_node % grid_size
        // col = index_node // grid_size
   
    // open_list.shift()
    // open_list.push({index_grid: beg_p.random_start, cost_function: 1000,traveled: 0, h: 1000 ,parent: NaN})
    let open_list = [{index_grid: beg_p.random_start, cost_function: 1000, traveled: 0, h: 1000, parent: NaN}]
    let closed_list = []
    ...
    while ( open_list.length > 0 && algo_working )
    {
        ...
        // while ( open_list[0]['cost_function'] != min_val )
        // {
            
            let working_node_idx = open_list[0]['index_grid']
            ...
            neighboors = []
            let index_node = working_node_idx                       
            ...
            {
                neighboors.push({index_grid: index_node-1, cost_function: 1+open_list[0]['traveled'] + Math.abs((index_node-1)%grid_size - row_end) + Math.abs((Math.floor((index_node-1)/grid_size))-col_end), traveled: 1+open_list[0]['traveled'], h: Math.abs((index_node-1)%grid_size - row_end) + Math.abs((Math.floor((index_node-1)/grid_size))-col_end), parrent: working_node_idx })
            }
            ...
            //up        
            neighboors.push({index_grid: index_node-grid_size, cost_function: 1+open_list[0]['traveled'] + Math.abs((index_node-grid_size)%grid_size - row_end) + Math.abs((Math.floor((index_node-grid_size)/grid_size)) - col_end), traveled: 1+open_list[0]['traveled'], h: Math.abs((index_node-grid_size)%grid_size - row_end) + Math.abs((Math.floor((index_node-grid_size)/grid_size)) - col_end), parrent: working_node_idx })            
            //down        
            neighboors.push({index_grid: index_node+grid_size, cost_function: 1+open_list[0]['traveled'] + Math.abs((index_node+grid_size)%grid_size - row_end) + Math.abs((Math.floor((index_node+grid_size)/grid_size)) - col_end), traveled: 1+open_list[0]['traveled'], h: Math.abs((index_node+grid_size)%grid_size - row_end) + Math.abs((Math.floor((index_node+grid_size)/grid_size)) - col_end), parrent: working_node_idx })
                ...
                for ( let j=1; j < open_list.length-i ; j++)
                {
                    if ( open_list[j-1]['cost_function'] >= open_list[j]['cost_function'] )
                    {
                        if (open_list[j-1]['cost_function'] > open_list[j]['cost_function'])
                        {
                            ...
                            if ( open_list[j-1]['h'] > open_list[j]['h'] )
                            {
                                let holder = open_list[j]
                                open_list[j] = open_list[j-1]
                                open_list[j-1] = holder  
        ...
        for ( let i=0; i<backprop_arr.length; i++)
        {
            if ( !document.getElementsByClassName('grid-item')[backprop_arr[i]['index_grid']].classList.contains('grid-item_start') )
            {
                ...
                {                    
                    ...
                    end_array.push(backprop_arr[i]['index_grid'])
                }
            }        
            if (!snake_game)
